...
const OrdersCount = ({ className, amountsOfOrders }) => {
  const lengths = {
    toSend: amountsOfOrders.filter(order => order.status === 1)
      .length,
    sended: amountsOfOrders.filter(order => order.status === 2)
      .length,
    delivered: amountsOfOrders.filter(order => order.status === 3)
      .length,
  };
  return (
...
OrdersCount.propTypes = {
  ...
  amountsOfOrders: PropTypes.oneOfType([PropTypes.array]),
};
OrdersCount.defaultProps = {
  className: '',
  amountsOfOrders: null,
};
